...
class MyApp(QMainWindow):
    ...
    def call_api_encrypt(self):
        ...
        payload = {
            "message": self.ui.plaintxt.toPlainText(), 
            "key_type": "public"
        }
        try:
            ...
            if response.status_code == 200:
                data = response.json()
                self.ui.ciphertxt.setPlainText(data["encrypted_message"])
                msg = QMessageBox()
                msg.setIcon(QMessageBox.Information)
    ...
    def call_api_decrypt(self):
        ...
        payload = {
            "ciphertext": self.ui.ciphertxt.toPlainText(),
            "key_type": "private"
        }
        try:
            response = requests.post(url, json=payload) 
            if response.status_code == 200:
                data = response.json()  
                self.ui.plaintxt.setPlainText(data["decrypted_message"])
                msg = QMessageBox() 
                msg.setIcon(QMessageBox.Information)
